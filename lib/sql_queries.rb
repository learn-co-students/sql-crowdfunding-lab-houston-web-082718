...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS amount_remaining
FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING amount_remaining >= 0;"
end
